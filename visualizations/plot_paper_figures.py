...
def fig1_density_advantage() -> None:
    ...
    # Annotations at 10% and 100%
    for i, row in df.iterrows():
        if row["density_pct"] in (10, 100):
            pct = row["delta_pct"]
            ci_val = ci[df.index.get_loc(i)]
            ax.annotate(
                f"+${row['delta_mean']:.2f}\n({pct:.1f}%)",
                xy=(row["density_pct"], row["delta_mean"] + ci_val + 0.08),
...
# ---------------------------------------------------------------------------
# Figure 2: Strategy comparison (mean profit with 95% CI)
# ---------------------------------------------------------------------------
def fig2_strategy_comparison() -> None:
    ...
    bars = ax.bar(x, means, yerr=ci, color=colors, capsize=5, edgecolor="black", linewidth=0.6)
    cs_mean = means[present.index("coldstart")] if "coldstart" in present else 0
    for i, (xi, m) in enumerate(zip(x, means)):
        delta = m - cs_mean
        sign = "+" if delta >= 0 else ""
        ax.text(xi, m + ci[i] + 0.4, f"{sign}${delta:.2f}", ha="center", va="bottom", fontsize=9)
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.set_ylabel("Mean profit per driver ($)")
    ax.set_title("Strategy comparison: mean profit with 95% CI (100% rider density, 5,000 drivers)")
    ax.axhline(cs_mean, color="gray", linewidth=0.8, linestyle="--", alpha=0.6)
    fig.tight_layout()
...
def fig3_profit_difference_distribution() -> None:
    ...
    ax = axes[0]
    bins = np.linspace(delta.quantile(0.01), delta.quantile(0.99), 50)
    ...
    ax.text(0.97, 0.95, f"Better: {pct_win:.1f}%\nWorse: {pct_lose:.1f}%\nTied: {pct_tie:.1f}%",
    ...
    ax = axes[1]
    by_cat = [merged[merged["route_length_category"] == c]["delta"] for c in CATEGORY_ORDER]
    bp = ax.boxplot(by_cat, tick_labels=[c.capitalize() for c in CATEGORY_ORDER], patch_artist=True)
...
def fig5_profit_by_density() -> None:
    ...
    ax.set_xlabel("Rider density (%)")
    ax.set_ylabel("Mean profit per driver ($)")
    ax.set_title("Mean profit by strategy across rider density")
